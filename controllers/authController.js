...
=======
      ...
      // CORREGIDO: Ahora compara contra user.clave_internet (nombre real de la columna en BD)
      if (!user || String(user.clave_internet) !== String(clave_internet)) {
        return res.status(401).json({ msg: "Credenciales inválidas" });
      }
      ...
      res.status(200).json({ 
        msg: "Paso 1 exitoso", 
        dni: user.dni, 
        nombre: user.nombre, // Retornamos el nombre dinámico desde la BD
        tokenDigitalSeed: user.token_digital_seed 
      });
    } catch (e) { 
      res.status(500).json({ error: e.message }); 
    }
>>>>>>> c2c4007 (Entrega Final)
  },
